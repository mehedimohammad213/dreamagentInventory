...
function resolveLocationState(): LocationState | null {
  return peekNavigationState<LocationState>();
}

const PurchaseHistoryEditor: React.FC = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const navStateHook = useNavigationState<LocationState>();
  ...
  const navState = resolveLocationState() ?? navStateHook;
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (isBulkEdit) {
        const records = resolveLocationState()?.records;
        if (!records?.length) {
  ...
  }, [id, isBulkEdit, isCreate, navigate, returnPurchaseTab]);
...
export default PurchaseHistoryEditor;
