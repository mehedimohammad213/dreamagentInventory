...
/** Prefer React location.state; fall back to sessionStorage (Next.js hydration lag). */
function resolveLocationState(
  locationState: unknown,
): LocationState | null {
  const fromLocation = locationState as LocationState | null;
  if (
    fromLocation &&
    (fromLocation.records?.length ||
      fromLocation.lcTemplate ||
      fromLocation.returnPurchaseTab)
  ) {
    return fromLocation;
  }
  return peekNavigationState<LocationState>();
}
...
const PurchaseHistoryEditorPage: React.FC = () => {
  ...
  const navState = resolveLocationState(location.state);

  const lcTemplateForCreate =
    isCreate ? navState?.lcTemplate ?? null : null;
  ...
  const returnPurchaseTab = navState?.returnPurchaseTab;
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (isBulkEdit) {
        const records = resolveLocationState(location.state)?.records;
        if (!records?.length) {
