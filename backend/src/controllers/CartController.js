...
export async function update(req, res) {
  try {
    const result = await CartService.updateCartItem(req.user.id, req.params.cart, req.body);
...
export async function destroy(req, res) {
  try {
    const result = await CartService.removeCartItem(req.user.id, req.params.cart);
    return success(res, result.data, result.message);
  } catch (err) {
