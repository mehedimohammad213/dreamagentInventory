...
export const uploadCategoryImage = multer({
  ...
  fileFilter: (_req, file, cb) => {
    const ok = /jpeg|jpg|png|gif/i.test(path.extname(file.originalname))
...
export const uploadCarAttachment = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    cb(ok ? null : new Error('Invalid attachment type'), ok);
  },
...
export const uploadExcel = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    cb(ok ? null : new Error('Invalid excel file'), ok);
  },
}).single('excel_file');
