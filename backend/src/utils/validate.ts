...
export function validate(
  ...
  for (const [field, ruleStr] of Object.entries(rules)) {
    ...
    const isNumericField = ruleList.some(
      (r) =>
        r === 'integer' ||
        r === 'numeric' ||
        r.startsWith('integer:') ||
        r.startsWith('numeric:')
    );
    ...
    for (const rule of ruleList) {
      ...
      if (name === 'required') {
        ...
        if (
          value != null &&
          !['true', 'false', '1', '0', true, false, 1, 0].includes(value as string | number | boolean)
        ) {
...
export function parseMaybeJson(value: unknown, fallback: unknown = null): unknown {
...
export function toInt(value: unknown, fallback: number | null = null): number | null {
  if (value === undefined || value === null || value === '') return fallback;
  const n = Number.parseInt(String(value), 10);
  return Number.isNaN(n) ? fallback : n;
}

export function toFloat(value: unknown, fallback: number | null = null): number | null {
  if (value === undefined || value === null || value === '') return fallback;
