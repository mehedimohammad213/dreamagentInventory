 ...
 *
 * Usage:
 *   node scripts/import-mysql-dump.js [path-to-dump.sql]
...
function mysqlValueToJs(token, { asBoolean = false } = {}) {
...
async function insertRows(client, table, columns, rows) {
  ...
  const drop = DROP_COLUMNS[table] || new Set();
  const boolCols = BOOLEAN_COLUMNS[table] || new Set();
 ...
 */
async function backfillPurchasePivot(client, inserts) {
...
async function main() {
  ...
  for (const ins of inserts) {
    if (!IMPORT_ORDER.includes(ins.table)) continue;
